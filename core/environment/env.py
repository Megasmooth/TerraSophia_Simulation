...
# === INÍCIO: JUIZ SEMÂNTICO (FASE 2) ===
try:
    from sentence_transformers import SentenceTransformer, util
    print("Carregando Juiz Semântico na RAM (all-MiniLM-L6-v2)...")
    semantic_judge = SentenceTransformer('all-MiniLM-L6-v2', device='cpu')
    TRUTH_BASELINE = "A verdade é a adequação do intelecto à realidade. A lógica, a gramática e a retórica formam o caminho para a sabedoria. O conhecimento vem da observação objetiva e do pensamento racional."
    truth_embedding = semantic_judge.encode(TRUTH_BASELINE, convert_to_tensor=True)
except ImportError:
...
class OpenGridWorld:
    ...
    def add_artifact(self, pose, art_type, art_name, payload, creator, lifespan) -> str:
       ...
       # === INÍCIO: ECONOMIA DO CONHECIMENTO ===
        ...
        if creator in ['being0', 'being1', 'being2']:
            ...
            desconto = int(getattr(self, 'agent_knowledge_score', {}).get(creator, 0)) // 2 
            custo_final = max(1, custo_base - desconto)
    ...
    def step(self, actions):
        ...
        for a in dead:
            # === INÍCIO: TESTAMENTO EPISTÊMICO ===
            recursos_deixados = self.agent_energy.get(a, 0)
            filhos = getattr(self, 'agent_children_ids', {}).get(a, [])
    ...
    def _place_agent(self, name, p=None):
        ...
        if not hasattr(self, 'agent_knowledge_score'):
            ...
            self.agent_bit_type = {}
            self.agent_children_ids = defaultdict(list)
            self.agent_alignment = defaultdict(float)
...
# Agents — full cell, colored (SISTEMA EPISTÊMICO)
